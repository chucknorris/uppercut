namespace uppercut.tests.infrastructure.containers.custom
{
    using System;
    using bdddoc.core;
    ...
    using WindsorContainer=uppercut.infrastructure.containers.custom.WindsorContainer;

    public class WindsorContainerSpecs
    {
        public abstract class concern_for_windsor_container :
            observations_for_a_sut_with_a_contract<IContainer, WindsorContainer>
        {
        ...
        {
            private static ILogFactory result;

            private context c =
                () => { the_container.Stub(x => x.Resolve<ILogFactory>()).Return(new Log4NetLogFactory()); };

            private because b = () => { result = sut.Resolve<ILogFactory>(); };
            ...
            concerns_using_a_real_container
        {
            ...
            private context c =
                () => { the_container.AddComponent("h", typeof (ILogFactory), typeof (Log4NetLogFactory)); };
            ...
            concerns_using_a_fake_container
        {
            ...
            private context c = () =>
                                    {
                                        the_container.Stub(x => x.Resolve<ILogFactory>()).Throw(
                                            new ComponentNotFoundException(typeof (ILogFactory)));
                                    };

            private because b =
                () => { attempting_to_get_an_unregistered_item = () => the_container.Resolve<ILogFactory>(); };
